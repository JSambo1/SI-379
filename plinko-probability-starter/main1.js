...
            const newY = initialY + easeOutQuad(progress) * (targetY - initialY);
...
            if(level === 0) {
                const topPegColor = chroma.scale(['#cccccc', 'darkred'])(1).hex();
...
                col--; // Go left
            }
            const randomFactor = 2 * Math.random() - 1; // Random number between -1 and 1
            const ballColor = chroma(BALL_COLOR).darken(randomFactor).saturate(randomFactor).hex();
    
...
            animateBallDrop(circle, y + GRAPH_HEIGHT, DELAY_WHEN_DROP / parseFloat(speedInput.value)) // Animate the ball dropping down
        ]);
    
